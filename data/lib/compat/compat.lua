...
-- THING_TYPE_DUELIST = CREATURETYPE_DUELIST + 1
...
do
	local function CreatureIndex(self, key)
		...
		elseif key == "type" then
			...
			elseif methods.isDuelist(self) then
				creatureType = THING_TYPE_MONSTER--THING_TYPE_DUELIST
			end
			return creatureType
	...
	rawgetmetatable("Duelist").__index = CreatureIndex
...
function isSummon(cid)
	...
	if creature.type == THING_TYPE_DUELIST then
		return false
	end
	return creature:getMaster() ~= nil
end
...
function MonsterType.getOutfit(self) return self:outfit() end
function MonsterType.getMaxHealth(self) return self:maxHealth() end
function MonsterType.getHealth(self) return self:health() end
function MonsterType.getName(self) return self:name() end
function MonsterType.getNameDescription(self) return self:nameDescription() end
...
Player.getUsingBall = Player.getUsingPokeball
