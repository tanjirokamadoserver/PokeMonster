...
local function register(self)
	if isScriptsInterface() then
		if not self.onReceive then
			print("[Warning - PacketHandler::register] need to setup a callback before you can register.")
			return
		end

		if type(self.onReceive) ~= "function" then
			print(string.format("[Warning - PacketHandler::onReceive] a function is expected."))
			return
		end
