...
combat:setStringParameter(COMBAT_PARAM_STRING_SPELLNAME, "Ice Storm")

-- Definindo direções para facilitar a navegação
...
-- Função para calcular a posição com base na direção
...
-- Função para lançar o efeito de "Ice Storm"
local function rotation_ring(parameters)
    ...
    if gir_table_up[parameters.n] ~= nil and gir_table_down[parameters.n] then
        ...
        -- Agendamento para continuar o efeito após um delay
...
-- Registrando o efeito de "Ice Storm"
local spell = Spell(SPELL_INSTANT)
function spell.onCastSpell(creature, variant)
    local center = creature:getPosition()
    local creatureId = creature:getId()

    -- Inicia o efeito "Ice Storm" com os parâmetros necessários
    rotation_ring({
