local combat = createCombatObject()
combat:setParameter(COMBAT_PARAM_TYPE, COMBAT_NORMALDAMAGE)
combat:setStringParameter(COMBAT_PARAM_STRING_SPELLNAME, "Bulk Up")

local condition = createConditionObject(CONDITION_DEFENSEBONUS)
condition:setParameter(CONDITION_PARAM_TICKS, 10000)
condition:setParameter(CONDITION_PARAM_EFFECT_TICKS, 2000)
condition:setParameter(CONDITION_PARAM_EFFECT, 55)

local condition2 = createConditionObject(CONDITION_RAGE)
condition2:setParameter(CONDITION_PARAM_TICKS, 10000)

local spell = Spell(SPELL_INSTANT)

function spell.onCastSpell(creature, variant)
    combat:execute(creature, variant) 
	creature:addCondition(condition)
	creature:addCondition2(condition2)
    return true
end

spell:name("Bulk Up")
spell:words("### Bulk Up ###")
spell:needLearn(false)
spell:register()
