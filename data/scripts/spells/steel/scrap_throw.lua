...
local MISSILE_EFFECT = 97

local combat = createCombatObject()
combat:setParameter(COMBAT_PARAM_TYPE, COMBAT_STEELDAMAGE)
combat:setStringParameter(COMBAT_PARAM_STRING_SPELLNAME, "Scrap Throw")
...
local function doShotDownDamage(creature_id, toPos, target_id)
    ...
    target:getPosition():sendMagicEffect(2141)
end
...
function spell.onCastSpell(creature, variant)
    ...
    local target = creature:getTarget()
    if not target then
        return false -- Caso o alvo não exista, não faça nada
    end
...
spell:name("Scrap Throw")
spell:words("#Scrap Throw#")
