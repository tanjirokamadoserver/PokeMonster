...
combat:setStringParameter(COMBAT_PARAM_STRING_SPELLNAME, "Venomous Gale")
...
function spell.onCastSpell(creature, variant)
    -- Efeito 813 na posição da criatura
    creature:getPosition():sendMagicEffect(813)

    -- Remover outfit
    local originalOutfit = creature:getOutfit()
    creature:setOutfit({lookType = 0}) -- Troca para outfit invisível
    ...
    -- Restaurar outfit original após o ataque
    addEvent(function()
        local caster = Creature(creatureId)
        if caster then
            caster:setOutfit(originalOutfit)
        end
    end, 4000) -- Ajuste o tempo (em milissegundos) conforme necessário

    return true
end

spell:name("Venomous Gale")
spell:words("#Venomous Gale#")
