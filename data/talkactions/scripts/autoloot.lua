function onSay(player, words, param)
    ...
    if action == "all" then
        local sto = player:getStorageValue(AUTOLOOT_ALL_ENABLED)
        if sto ~= 1 then
            player:setStorageValue(AUTOLOOT_ALL_ENABLED, 1)
            player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Você ativou o autoloot all")
        else
            player:setStorageValue(AUTOLOOT_ALL_ENABLED, -1)
            player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Você desativou o autoloot all")
        end
        return false
    end

    if action == "add" then
        if not split[2] then return player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "There is no item with that id or name.") and false end
    ...
    elseif action == "clear" then
        ...
        player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Use the commands: !autoloot {add, remove, show, clear, all}")
    end
