...
Player::Player(ProtocolGame_ptr p) :
    Creature(), client(std::move(p)), lastPing(OTSYS_TIME()), lastPong(lastPing), inbox(new Inbox(ITEM_INBOX))
{
	...
	if (index == INDEX_WHEREEVER) {
		...
		for (int32_t slotIndex = CONST_SLOT_FIRST; slotIndex <= CONST_SLOT_LAST; ++slotIndex) {
			...
			if (inventoryItem) {
				...
				} else if (inventoryItem->isStackable() && item->equals(inventoryItem) && inventoryItem->getItemCount() < MAX_ITEM_COUNT) {
					uint32_t remainder = (MAX_ITEM_COUNT - inventoryItem->getItemCount());
		...
		if (destItem) {
			if (destItem->isStackable() && item->equals(destItem) && destItem->getItemCount() < MAX_ITEM_COUNT) {
				maxQueryCount = MAX_ITEM_COUNT - destItem->getItemCount();
			} else {
				maxQueryCount = 0;
			}
		} else if (queryAdd(index, *item, count, flags) == RETURNVALUE_NOERROR) { //empty slot
			if (item->isStackable()) {
				maxQueryCount = MAX_ITEM_COUNT;
			} else {
				maxQueryCount = 1;
			}
		...
		*destItem = nullptr;
			...
			if (inventoryItem) {
				...
				if (autoStack && isStackable) {
					...
					if (queryAdd(slotIndex, *item, item->getItemCount(), 0) == RETURNVALUE_NOERROR) {
						if (inventoryItem->equals(item) && inventoryItem->getItemCount() < MAX_ITEM_COUNT) {
							index = slotIndex;
		...
		while (i < containers.size()) {
			...
			for (Item* tmpItem : tmpContainer->getItemList()) {
				...
				//try find an already existing item to stack with
				if (tmpItem->equals(item) && tmpItem->getItemCount() < MAX_ITEM_COUNT) {
					index = n;
					*destItem = tmpItem;
					return tmpContainer;
				}
...
Skulls_t Player::getSkullClient(const Creature* creature) const
{
	...
	if (!player->getGuild()->isInAnyWar() && guild == player->getGuild()) {
		return SKULL_GREEN;
	}
...
{
	if (isAccessPlayer() &&  getAccountType() >= ACCOUNT_TYPE_GAMEMASTER) {
		return true;
	}
...
void Player::removeGuildBuffs()
{
	...
	if (guild->hasBuff(COLUMN_3, PLAYER_SPEED_BUFF)) {
		g_game.changeSpeed(this, -(GUILD_BUFF_PLAYER_SPEED*2));
	}

	if (getSummons().size() > 0) {
		for (const auto& summon : getSummons()) {
			if (guild->hasBuff(COLUMN_3, POKEMON_SPEED_BUFF)) {
				g_game.changeSpeed(summon, -(GUILD_BUFF_POKEMON_SPEED));
			}

			if (guild->hasBuff(COLUMN_6, POKEMON_HEALTH_BUFF)) {
				int64_t newMaxHealth = (summon->oldMaxHealth * GUILD_BUFF_POKEMON_HEALTH / 100);
				summon->healthMax = summon->getMaxHealth() + newMaxHealth;
				summon->health = std::min<int64_t>(summon->health, summon->healthMax);
				g_game.addCreatureHealth(summon);
			}
		}
	}
...
void Player::applyGuildBuffs()
{
	...
	if (getSummons().size() > 0) {
		for (const auto& summon : getSummons()) {
			...
			if (guild->hasBuff(COLUMN_6, POKEMON_HEALTH_BUFF)) {
				summon->oldMaxHealth = summon->getMaxHealth();

				int64_t newMaxHealth = (summon->getMaxHealth() * GUILD_BUFF_POKEMON_HEALTH / 100);
				summon->healthMax = summon->getMaxHealth() + newMaxHealth;
				summon->health = std::min<int64_t>(summon->health, summon->healthMax);
				g_game.addCreatureHealth(summon);
			}
...
void Player::setSpectating(Player* target) {
	if (spectating == target || target == this) {
		return;
	}
	if (spectating) {
		...
		g_game.playerCancelAttackAndFollow(getID());
		spectating = target;
	}
	if (target) {
		target->addSpectator(this);
	    if (client)
	      client->sendStartSpectating(target);
	}
}