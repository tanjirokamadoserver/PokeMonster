...
{
	//onHealthChange(creature, attacker, primaryDamage, primaryType, origin)
	if (!scriptInterface->reserveScriptEnv()) {
	...
	if (scriptInterface->protectedCall(L, 6, 4) != 0) {
		...
		damage.critical = LuaScriptInterface::getBoolean(L, -2);
...
void CreatureEvent::executeManaChange(Creature* creature, Creature* attacker, CombatDamage& damage) {
	//onManaChange(creature, attacker, primaryDamage, primaryType, origin)
	if (!scriptInterface->reserveScriptEnv()) {
	...
	if (scriptInterface->protectedCall(L, 7, 4) != 0) {
		...
		damage.primary.value = LuaScriptInterface::getNumber<int32_t>(L, -3);
		damage.primary.type = LuaScriptInterface::getNumber<CombatType_t>(L, -2);
		damage.spellName = LuaScriptInterface::getString(L, -1);
		lua_pop(L, 3);
	}
