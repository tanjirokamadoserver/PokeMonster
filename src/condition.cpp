...
bool Condition::setParam(ConditionParam_t param, int32_t value)
{
	switch (param) {
		...
		case CONDITION_PARAM_SAVE_CONDITION: {
			saveCondition = (value != 0);
			return true;
		}
		...
		case CONDITION_PARAM_EFFECT_TICKS: {
			addEffTicks = value;
			return true;
		}
...
bool Condition::sendEff(Creature* creature, uint16_t effectId)
{
	if(ticks > 0 && effectId != 0) {
		const Position& creaturePos = creature->getPosition();
		g_game.addMagicEffect(creaturePos, effectId);
		return true;
	}
...
bool Condition::executeCondition(Creature* creature, int32_t interval)
{
    ...
    ConditionDamage* damageCondition = dynamic_cast<ConditionDamage*>(this);
    if (!damageCondition && addEff > 0) {
        int64_t totalCheck = OTSYS_TIME() - lastEffTicks;
        if (totalCheck >= addEffTicks) {
            lastEffTicks = OTSYS_TIME();
            sendEff(creature, addEff);
        }
    }
...
Condition* Condition::createCondition(ConditionId_t id, ConditionType_t type, int32_t ticks, int32_t param/* = 0*/, bool buff/* = false*/, uint32_t subId/* = 0*/, bool aggressive/* = false */)
{
	switch (type) {
		...
		case CONDITION_MOVING: //pota
			return new ConditionStatus(id, type, ticks, buff, subId, aggressive);
		...
		case CONDITION_RAGE:
		case CONDITION_MELEE_RAGE:
		case CONDITION_MANASHIELD:
			return new ConditionGeneric(id, type, ticks, buff, subId, aggressive);

		case CONDITION_ATTACKBONUS:
		case CONDITION_DEFENSEBONUS:
			return new ConditionBonus(id, type, ticks, buff, subId, aggressive);
...
bool ConditionDamage::addDamage(int64_t rounds, int64_t time, int64_t value)
{
	time = std::max<int64_t>(time, EVENT_CREATURE_THINK_INTERVAL);
	if (rounds == -1) {
		//periodic damage
		periodDamage = value;
...
bool ConditionDamage::doDamage(Creature* creature, int64_t healthChange)
{
	...
	if (attacker && attacker->getMaster() && attacker->getMaster()->getPlayer()) {
		if (Player* player = attacker->getMaster()->getPlayer()) {
			Item* pokeball = player->getUsingPokeball();
			if (pokeball) {
				uint16_t heldX = pokeball->getIntAttr(ITEM_ATTRIBUTE_HELDX);
				if (mapHeldPoison.find(heldX) != mapHeldPoison.end()) { // poison held
					int16_t bonus = mapHeldPoison[heldX];
					damage.primary.value += ((damage.primary.value*bonus)/100);
				} else if (mapHeldHellfire.find(heldX) != mapHeldHellfire.end()) { // hellfire held
					int16_t bonus = mapHeldHellfire[heldX];
					damage.primary.value += ((damage.primary.value*bonus)/100);
				}
			}
		}
...
void ConditionStatus::addCondition([[maybe_unused]] Creature* creature, const Condition* addCondition)
{
...
bool ConditionBonus::startCondition(Creature* creature)
{
	return Condition::startCondition(creature);
}

bool ConditionBonus::executeCondition(Creature* creature, int32_t interval)
{
	return Condition::executeCondition(creature, interval);
}

void ConditionBonus::endCondition(Creature*)
{
	//
}

void ConditionBonus::addCondition(Creature*, const Condition* addCondition)
{
	if (updateCondition(addCondition)) {
		setTicks(addCondition->getTicks());
	}
...
uint32_t ConditionBonus::getIcons() const
{
	uint32_t icons = Condition::getIcons();
