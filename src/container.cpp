	...
	if (item->isStackable()) {
		...
		if (index == INDEX_WHEREEVER) {
			...
			for (Item* containerItem : itemlist) {
				if (containerItem != item && containerItem->equals(item) && containerItem->getItemCount() < MAX_ITEM_COUNT) {
					uint32_t remainder = (MAX_ITEM_COUNT - containerItem->getItemCount());
			...
			if (item->equals(destItem) && destItem->getItemCount() < MAX_ITEM_COUNT) {
				uint32_t remainder = MAX_ITEM_COUNT - destItem->getItemCount();
		...
		maxQueryCount = freeSlots * MAX_ITEM_COUNT + n;
		...
		uint32_t& flags)
{
	...
	if (autoStack && item->isStackable() && item->getParent() != this) {
		...
		for (Item* listItem : itemlist) {
			if (listItem != item && listItem->equals(item) && listItem->getItemCount() < MAX_ITEM_COUNT) {
				*destItem = listItem;
				index = n;
				return this;
			}
...
void Container::addThing(int32_t index, Thing* thing)
{
	...
	Player* player = dynamic_cast<Player*>(getTopParent());
	if (itemType.isPokeballItem()) {
		if (player) {
			player->insertPokeball(item);
		}
		insertPokeball(item);
	} else if (Container* itemContainer = dynamic_cast<Container*>(item)) {
		for (Item* pokeball : itemContainer->getPokeballsCached()) {
			insertPokeball(pokeball);
			if (player) {
				player->insertPokeball(pokeball);
			}
		}
	}
...
void Container::removeThing(Thing* thing, uint32_t count)
{
	...
	if (item->isStackable() && count != item->getItemCount()) {
		...
		if (itemType.isPokeballItem()) {
			if (player) {
				player->removeBallFromCache(item);
			}
			removeBallFromCache(item);
		} else if (Container* itemContainer = dynamic_cast<Container*>(item)) {
			for (Item* pokeball : itemContainer->getPokeballsCached()) {
				removeBallFromCache(pokeball);
				if (player) {
					player->removeBallFromCache(pokeball);
				}
			}
		}
	}
...
{
	if (this->hasAttribute(ITEM_ATTRIBUTE_UNIQUE_ITEM)) {
		return true;
	}

	for (ContainerIterator it = iterator(); it.hasNext(); it.advance()) {
		if ((*it)->hasAttribute(ITEM_ATTRIBUTE_UNIQUE_ITEM)) {
			return true;
		}
	}
	return false;
