...
bool Events::load()
{
	...
	for (auto eventNode : doc.child("events").children()) {
		...
		if (className == "Monster") { //pota
			...
			} else if(methodName == "onCheckImmunity") {
				info.monsterOnCheckImmunity = event;
			} else {
				std::cout << "[Warning - Events::load] Unknown monster method: " << methodName << std::endl;
			}
		} else if (className == "Duelist") {
				// TODO: Implement Duelist events
				// onDropLoot
			...
			if (methodName == "onBrowseField") {
 			...
 			} else if (methodName == "onLostConnection") {
				info.playerOnLostConnection = event;
...
bool Events::eventCreatureOnChangeZone(Creature* creature, Zone* oldZone, Zone* newZone, bool isTeleport)
{
	// Creature:onChangeZone(old, new)
	...
	LuaScriptInterface::pushUserdata<Zone>(L, oldZone);
	LuaScriptInterface::setMetatable(L, -1, "Zone");
	
	LuaScriptInterface::pushUserdata<Zone>(L, newZone);
	LuaScriptInterface::setMetatable(L, -1, "Zone");
...
bool Events::eventMonsterOnCheckImmunity(const Monster* monster, CombatType_t combatType, ConditionType_t conditionType)
{
	// Monster:onCheckImmunity(combatType, conditionType)
	if (info.monsterOnCheckImmunity == -1) {
		return false;
	}

	if (!scriptInterface.reserveScriptEnv()) {
		std::cout << "[Error - Events::eventMonsterOnCheckImmunity] Call stack overflow" << std::endl;
		return false;
	}

	ScriptEnvironment* env = scriptInterface.getScriptEnv();
	env->setScriptId(info.monsterOnCheckImmunity, &scriptInterface);

	lua_State* L = scriptInterface.getLuaState();
	scriptInterface.pushFunction(info.monsterOnCheckImmunity);

	LuaScriptInterface::pushUserdata<Monster>(L, const_cast<Monster*>(monster));
	LuaScriptInterface::setMetatable(L, -1, "Monster");

	lua_pushnumber(L, combatType);
	lua_pushnumber(L, conditionType);
...
void Events::eventPlayerOnLostConnection(Player* player)
{
	// Player:onLostConnection()
	if (info.playerOnLostConnection == -1) {
		return;
	}
	if (!scriptInterface.reserveScriptEnv()) {
		std::cout << "[Error - Events::eventPlayerOnLostConnection] Call stack overflow" << std::endl;
		return;
	}
	ScriptEnvironment* env = scriptInterface.getScriptEnv();
	env->setScriptId(info.playerOnLostConnection, &scriptInterface);
	lua_State* L = scriptInterface.getLuaState();
	scriptInterface.pushFunction(info.playerOnLostConnection);
	LuaScriptInterface::pushUserdata<Player>(L, player);
	LuaScriptInterface::setMetatable(L, -1, "Player");
