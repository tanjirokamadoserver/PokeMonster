...
const std::unordered_map<std::string, ItemParseAttributes_t> ItemParseAttributesMap = {
	...
	{"marketcategory", ITEM_PARSE_MARKETCATEGORY},
	{"moveable", ITEM_PARSE_MOVEABLE},
	{"movable", ITEM_PARSE_MOVEABLE},
	{"blockprojectile", ITEM_PARSE_BLOCKPROJECTILE},
	{"ispokeball", ITEM_PARSE_ISPOKEBALL},
	{"isholder", ITEM_PARSE_ISHOLDER},
	...
	for (auto attributeNode : itemNode.children()) {
		...
		if (parseAttribute != ItemParseAttributesMap.end()) {
			...
			switch (parseType) {
				...
				case ITEM_PARSE_MARKETCATEGORY: {
					it.marketCategory = pugi::cast<int32_t>(valueAttribute.value());
					break;
				}
				case ITEM_PARSE_ISPOKEBALL: {
					it.isPokeball = pugi::cast<int32_t>(valueAttribute.value());
					break;
				}
				case ITEM_PARSE_ISHOLDER: {
					it.isHolder = pugi::cast<int32_t>(valueAttribute.value());
					break;
				}
