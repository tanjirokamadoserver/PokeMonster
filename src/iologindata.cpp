...
bool IOLoginData::loadPlayerById(Player* player, uint32_t id)
{
	Database& db = Database::getInstance();
	std::ostringstream query;
	query << "SELECT `id`, `name`, `account_id`, `group_id`, `sex`, `vocation`, `experience`, `level`, `maglevel`, `health`, `healthmax`, `blessings`, `mana`, `manamax`, `manaspent`, `soul`, `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype`, `lookaddons`, `posx`, `posy`, `posz`, `cap`, `lastlogin`, `lastlogout`, `lastip`, `conditions`, `skulltime`, `skull`, `town_id`, `balance`, `offlinetraining_time`, `offlinetraining_skill`, `stamina`, `skill_fist`, `skill_fist_tries`, `skill_club`, `skill_club_tries`, `skill_sword`, `skill_sword_tries`, `skill_axe`, `skill_axe_tries`, `skill_dist`, `skill_dist_tries`, `skill_shielding`, `skill_shielding_tries`, `skill_fishing`, `skill_fishing_tries`, `lookaura`, `lookshader`, `lookwings` FROM `players` WHERE `id` = " << id;
...
bool IOLoginData::loadPlayerByName(Player* player, const std::string& name)
{
	Database& db = Database::getInstance();
	std::ostringstream query;
	query << "SELECT `id`, `name`, `account_id`, `group_id`, `sex`, `vocation`, `experience`, `level`, `maglevel`, `health`, `healthmax`, `blessings`, `mana`, `manamax`, `manaspent`, `soul`, `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype`, `lookaddons`, `posx`, `posy`, `posz`, `cap`, `lastlogin`, `lastlogout`, `lastip`, `conditions`, `skulltime`, `skull`, `town_id`, `balance`, `offlinetraining_time`, `offlinetraining_skill`, `stamina`, `skill_fist`, `skill_fist_tries`, `skill_club`, `skill_club_tries`, `skill_sword`, `skill_sword_tries`, `skill_axe`, `skill_axe_tries`, `skill_dist`, `skill_dist_tries`, `skill_shielding`, `skill_shielding_tries`, `skill_fishing`, `skill_fishing_tries`, `lookaura`, `lookshader`, `lookwings` FROM `players` WHERE `name` = " << db.escapeString(name);
	return loadPlayer(player, db.storeQuery(query.str()));
}
...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
	...
	// auras, shaders, wings new loading
	...
	if ((result = db.storeQuery(query.str()))) {
		...
		player->guildInvites.push_back(guildId);
	}

	query.str(std::string());
	query << "SELECT `guild_id`, `rank_id`, `nick`, `contribution` FROM `guild_members` WHERE `player_id` = " << player->getGUID();
	if ((result = db.storeQuery(query.str()))) {
		uint32_t guildId = result->getNumber<uint32_t>("guild_id");
	...
	//load string storages map
	query.str(std::string());
	query << "SELECT `key`, `value` FROM `players_stringstorages` WHERE `player_id` = " << player->getGUID();
	if ((result = db.storeQuery(query.str()))) {
		do {
			player->addStringStorageValue(result->getNumber<uint32_t>("key"), result->getString("value"));
		} while (result->next());
	}
	//load pokeball brokes
	query.str(std::string());
	query << "SELECT `pokemonName`, `poke`, `great`, `ultra`, `saffari`, `master`, `moon`, `tinker`, `sora`, `dusk`, `yume`, `tale`, `net`, `janguru`, `magu`, `fast`, `heavy`, `premier`, `delta`, `esferadepal`, `esferamega`, `esferagiga`, `esferatera`, `esferaultra`, `esferalendaria`, `super`, `especial`, `divine` FROM `pokeball_stats` WHERE `player_id` = " << player->getGUID();
	if ((result = db.storeQuery(query.str()))) {
		do {
			pokeballsInfo brokes;
			std::string pokeName 		 = result->getString("pokemonName");
			brokes.poke 			     = result->getNumber<uint32_t>("poke");
			brokes.great				 = result->getNumber<uint32_t>("great");
			brokes.ultra 				 = result->getNumber<uint32_t>("ultra");
			brokes.saffari 				 = result->getNumber<uint32_t>("saffari");
			brokes.master 				 = result->getNumber<uint32_t>("master");
			brokes.moon					 = result->getNumber<uint32_t>("moon");
			brokes.tinker 				 = result->getNumber<uint32_t>("tinker");
			brokes.sora					 = result->getNumber<uint32_t>("sora");
			brokes.dusk					 = result->getNumber<uint32_t>("dusk");
			brokes.yume 				 = result->getNumber<uint32_t>("yume");
			brokes.tale					 = result->getNumber<uint32_t>("tale");
			brokes.net 					 = result->getNumber<uint32_t>("net");
			brokes.janguru				 = result->getNumber<uint32_t>("janguru");
			brokes.magu 				 = result->getNumber<uint32_t>("magu");
			brokes.fast					 = result->getNumber<uint32_t>("fast");
			brokes.heavy				 = result->getNumber<uint32_t>("heavy");
			brokes.premier 				 = result->getNumber<uint32_t>("premier");
			brokes.delta 			     = result->getNumber<uint32_t>("delta");
			brokes.esferadepal 			 = result->getNumber<uint32_t>("esferadepal");
			brokes.esferamega 			 = result->getNumber<uint32_t>("esferamega");
			brokes.esferagiga 			 = result->getNumber<uint32_t>("esferagiga");
			brokes.esferatera 			 = result->getNumber<uint32_t>("esferatera");
			brokes.esferaultra			 = result->getNumber<uint32_t>("esferaultra");
			brokes.esferalendaria		 = result->getNumber<uint32_t>("esferalendaria");
			brokes.super		 		 = result->getNumber<uint32_t>("super");
			brokes.especial		 		 = result->getNumber<uint32_t>("especial");
			brokes.divine		 		 = result->getNumber<uint32_t>("divine");

			player->loadPokeballsBrokes(pokeName, brokes);
		} while (result->next());
	}


	query.str(std::string());
	query << "SELECT `pokemonName`, `pontos` FROM `pokemon_points` WHERE `player_id` = " << player->getGUID();
	if ((result = db.storeQuery(query.str()))) {
		do {
			std::string pokeName 		 = result->getString("pokemonName");
			uint16_t pontos			 	 = result->getNumber<uint16_t>("pontos");
			player->loadCatchPoints(pokeName, pontos);
		} while (result->next());
	}
	...
	std::string inboxQuery = fmt::format(
		"SELECT inbox.id, inbox.target_id, inbox.guild_id, inbox.date, inbox.type, inbox.text, inbox.finished "
		"FROM guilds_inbox AS inbox "
		"JOIN guilds_player_inbox AS p_inbox ON inbox.id = p_inbox.inbox_id "
		"WHERE p_inbox.player_id = {:d} ORDER BY inbox.date DESC;",
		player->getGUID()
	);

	if ((result = db.storeQuery(inboxQuery))) {
		do {
...
bool IOLoginData::saveItems(const Player* player, const ItemBlockList& itemList, DBInsert& query_insert, PropWriteStream& propWriteStream, std::map<Container*, int>& openContainers)
{
	...
	for (const auto& it : itemList) {
		...
		if (Container* container = item->getContainer()) {
			auto it_ = openContainers.find(container);
			if (it_ == openContainers.end()) {
				container->resetAutoOpen();
			} else {
				container->setAutoOpen(it_->second);
			}
...
bool IOLoginData::savePlayer(Player* player)
{
	...
	query.str(std::string());
	DBInsert guildInvitesQuery("INSERT INTO `guild_invites` (`player_id`, `guild_id`) VALUES ");

	for (const auto& it : player->guildInvites) {
		query << player->getGUID() << ',' << it;
		if (!guildInvitesQuery.addRow(query)) {
			return false;
		}
	}

	if (!guildInvitesQuery.execute()) {
		return false;
	}
	query.str(std::string());
	...
	query << "DELETE FROM `players_stringstorages` WHERE `player_id` = " << player->getGUID();
	if (!db.executeQuery(query.str())) {
		return false;
	}
	...
	DBInsert stringStorageQuery("INSERT INTO `players_stringstorages` (`player_id`, `key`, `value`) VALUES ");

	for (const auto& it : player->stringStorageMap) {
		query << player->getGUID() << ',' << it.first << ',' << db.escapeString(it.second);
		if (!stringStorageQuery.addRow(query)) {
			return false;
		}
	}

	if (!stringStorageQuery.execute()) {
		return false;
	}

	query.str(std::string());
	query << "DELETE FROM `pokeball_stats` WHERE `player_id` = " << player->getGUID();
	if (!db.executeQuery(query.str())) {
		return false;
	}

	query.str(std::string());
	DBInsert pokeballsBrokes("INSERT INTO `pokeball_stats` (`player_id`, `pokemonName`, `poke`, `great`, `ultra`, `saffari`, `master`, `moon`, `tinker`, `sora`, `dusk`, `yume`, `tale`, `net`, `janguru`, `magu`, `fast`, `heavy`, `premier`, `delta`, `esferadepal`, `esferamega`, `esferagiga`, `esferatera`, `esferaultra`, `esferalendaria`, `super`, `especial`, `divine`) VALUES ");

	for (const auto& it : player->playerBrokes) {
		pokeballsInfo brokes = it.second;
		query << fmt::format("{:d}, {:s}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}, {:d}",
        player->getGUID(), db.escapeString(it.first), brokes.poke, brokes.great, brokes.ultra, brokes.saffari, brokes.master, brokes.moon, brokes.tinker,
        brokes.sora, brokes.dusk, brokes.yume, brokes.tale, brokes.net, brokes.janguru, brokes.magu, brokes.fast, brokes.heavy, brokes.premier,
        brokes.delta, brokes.esferadepal, brokes.esferamega, brokes.esferagiga, brokes.esferatera, brokes.esferaultra, brokes.esferalendaria, brokes.super, brokes.especial, brokes.divine);
    
		if (!pokeballsBrokes.addRow(query)) {
			return false;
		}
	}

	if (!pokeballsBrokes.execute()) {
		return false;
	}

	query.str(std::string());
	query << "DELETE FROM `pokemon_points` WHERE `player_id` = " << player->getGUID();
	if (!db.executeQuery(query.str())) {
		return false;
	}

	query.str(std::string());
	DBInsert catchPoints("INSERT INTO `pokemon_points` (`player_id`, `pokemonName`, `pontos`) VALUES ");

	for (const auto& it : player->playerPoints) {
		query << fmt::format("{:d}, {:s}, {:d}",
        player->getGUID(), db.escapeString(it.first), it.second);
    
		if (!catchPoints.addRow(query)) {
			return false;
		}
	}

	if (!catchPoints.execute()) {
		return false;
	}
...
bool IOLoginData::getInfoForCharacter(const std::string& characterName, CharacterInfo& info)
{
    Database& db = Database::getInstance();

    std::ostringstream query;
    query << "SELECT `vocation`, `level`, `looktype`, `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `lookaddons`, `pokemons` FROM `players` WHERE `name` = " << db.escapeString(characterName);
    ...
    if (result) {
        info.vocation 			= result->getNumber<uint32_t>("vocation");
        info.level 				= result->getNumber<uint32_t>("level");
        info.looktype		    = result->getNumber<uint32_t>("looktype");
        info.lookbody 			= result->getNumber<uint32_t>("lookbody");
        info.lookfeet 			= result->getNumber<uint32_t>("lookfeet");
        info.lookhead 			= result->getNumber<uint32_t>("lookhead");
        info.looklegs 			= result->getNumber<uint32_t>("looklegs");
        info.lookaddons			= result->getNumber<uint32_t>("lookaddons");
        info.pokemons 			= result->getString("pokemons");
        return true;
    } else {
        return false;
    }
