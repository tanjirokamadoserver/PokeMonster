...
class Creature : virtual public Thing
{
	...
	public:
		...
		virtual Duelist* getDuelist() {
			return nullptr;
		}
		virtual const Duelist* getDuelist() const {
			return nullptr;
		}
		...
		void setNameColor(NameColor_t newColor);
		...
		virtual NameColor_t getNameColorClient(const Creature* creature) const {
			return creature->getNameColor();
		}
		...
		void setBossStatus(bool value) {
			isBoss = value;
		}
		
		bool isBossReturn() const {
			return isBoss;
		}
		...
		void addGuardian(Creature* guardian);
		void removeGuardian(Creature* guardian);
		const std::list<Creature*>& getGuardians() const {
			return guardians;
		}

		bool isMonsterGuardian() {
			return isGuardian;
		}
		bool isMonsterGuardian() const {
			return isGuardian;
		}
		...
		bool isImmuneEvent(CombatType_t combatType, ConditionType_t conditionType);
		...
		const std::map<std::string, std::pair<int16_t, int16_t>>& getIconMap() const;
		void insertIcon(const std::string& iconName, int16_t pointX, int16_t pointY);
    	void removeIcon(const std::string& iconName);
		void cleanIcons();
       	void addStringStorageValue(const uint32_t key, const std::string value);
		bool getStringStorageValue(const uint32_t key, std::string& value) const;
		
		std::map<uint32_t, std::string> stringStorageMap;
		...
		NameColor_t nameColor = NameColor_t(9);
		
		const std::string& getTitle() const { return title; }
	        
		const std::string& getTitleFont() const { return titleFont; }
		
		const std::string& getTitleColor() const { return titleColor; }

		void clearListWalkDir() { listWalkDir.clear(); }
		
		int64_t oldMaxHealth = 0;

		AttachedEffMap getAttachedEffects() const { return attachedEffects; }

		void insertAttachedEffect(uint16_t effectId, uint8_t front);
		void removeAttachedEffect(uint16_t effectId);
		void cleanAttachedEffects();
		void refreshTile();
	...
	protected:
		...
		AttachedEffMap attachedEffects;
		CountMap damageMap;
