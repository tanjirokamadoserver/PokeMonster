...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
    	...
    	case ATTR_UNIQUE_ITEM: {
    	    int32_t uniqueId;
    	    if (!propStream.read<int32_t>(uniqueId)) {
    	        return ATTR_READ_ERROR;
    	    } 
    	    setIntAttr(ITEM_ATTRIBUTE_UNIQUE_ITEM, uniqueId);
    	    break;
    	}

    	case ATTR_POKEMONHOLDER: {
    	    std::string pokemonHolder;
    	    if (!propStream.readString(pokemonHolder)) {
    	        return ATTR_READ_ERROR;
    	    } 
    	    setStrAttr(ITEM_ATTRIBUTE_POKEMONHOLDER, pokemonHolder);
    	    break;
    	}

		case ATTR_POKEMONHOLDERINFO: {
    	    std::string pokemonHolder;
    	    if (!propStream.readString(pokemonHolder)) {
    	        return ATTR_READ_ERROR;
    	    } 
    	    setStrAttr(ITEM_ATTRIBUTE_POKEMONHOLDERINFO, pokemonHolder);
    	    break;
    	}

		case ATTR_POKEMONHOLDERCREATUREID: {
    	    int64_t creatureId;
    	    if (!propStream.read<int64_t>(creatureId)) {
    	        return ATTR_READ_ERROR;
    	    } 
    	    setIntAttr(ITEM_ATTRIBUTE_POKEMONHOLDERCREATUREID, creatureId);
    	    break;
    	}
		case ATTR_ATTRIBUTE_HELDX: {
    	    int32_t attributeIdent;
    	    if (!propStream.read<int32_t>(attributeIdent)) {
    	        return ATTR_READ_ERROR;
    	    } 
    	    setIntAttr(ITEM_ATTRIBUTE_HELDX, attributeIdent);
    	    break;
    	}
		case ATTR_ATTRIBUTE_HELDY: {
    	    int32_t attributeIdent;
    	    if (!propStream.read<int32_t>(attributeIdent)) {
    	        return ATTR_READ_ERROR;
    	    } 
    	    setIntAttr(ITEM_ATTRIBUTE_HELDY, attributeIdent);
    	    break;
    	}
		case ATTR_ATTRIBUTE_HELDU: {
    	    int32_t attributeIdent;
    	    if (!propStream.read<int32_t>(attributeIdent)) {
    	        return ATTR_READ_ERROR;
    	    } 
    	    setIntAttr(ITEM_ATTRIBUTE_HELDU, attributeIdent);
    	    break;
    	}
		case ATTR_ATTRIBUTE_BLINK: {
    	    int64_t valorAttr;
    	    if (!propStream.read<int64_t>(valorAttr)) {
    	        return ATTR_READ_ERROR;
    	    } 
    	    setIntAttr(ITEM_ATTRIBUTE_BLINK, valorAttr);
    	    break;
    	}
...
void Item::serializeAttr(PropWriteStream& propWriteStream) const
{
    ...
    if (hasAttribute(ITEM_ATTRIBUTE_UNIQUE_ITEM)) {
        propWriteStream.write<uint8_t>(ATTR_UNIQUE_ITEM);
        propWriteStream.write<uint32_t>(getIntAttr(ITEM_ATTRIBUTE_UNIQUE_ITEM));
    }
    if (hasAttribute(ITEM_ATTRIBUTE_POKEMONHOLDER)) {
        propWriteStream.write<uint8_t>(ATTR_POKEMONHOLDER);
        propWriteStream.writeString(getStrAttr(ITEM_ATTRIBUTE_POKEMONHOLDER));
    }

    if (hasAttribute(ITEM_ATTRIBUTE_POKEMONHOLDERINFO)) {
        propWriteStream.write<uint8_t>(ATTR_POKEMONHOLDERINFO);
        propWriteStream.writeString(getStrAttr(ITEM_ATTRIBUTE_POKEMONHOLDERINFO));
    }
    if (hasAttribute(ITEM_ATTRIBUTE_POKEMONHOLDERCREATUREID)) {
        propWriteStream.write<uint8_t>(ATTR_POKEMONHOLDERCREATUREID);
        propWriteStream.write<uint64_t>(getIntAttr(ITEM_ATTRIBUTE_POKEMONHOLDERCREATUREID));
    }
    if (hasAttribute(ITEM_ATTRIBUTE_HELDY)) {
        propWriteStream.write<uint8_t>(ATTR_ATTRIBUTE_HELDY);
        propWriteStream.write<uint32_t>(getIntAttr(ITEM_ATTRIBUTE_HELDY));
    }
    if (hasAttribute(ITEM_ATTRIBUTE_HELDX)) {
        propWriteStream.write<uint8_t>(ATTR_ATTRIBUTE_HELDX);
        propWriteStream.write<uint32_t>(getIntAttr(ITEM_ATTRIBUTE_HELDX));
    }
    if (hasAttribute(ITEM_ATTRIBUTE_HELDU)) {
        propWriteStream.write<uint8_t>(ATTR_ATTRIBUTE_HELDU);
        propWriteStream.write<uint32_t>(getIntAttr(ITEM_ATTRIBUTE_HELDU));
    }
    if (hasAttribute(ITEM_ATTRIBUTE_BLINK)) {
        propWriteStream.write<uint8_t>(ATTR_ATTRIBUTE_BLINK);
        propWriteStream.write<uint64_t>(getIntAttr(ITEM_ATTRIBUTE_BLINK));
    }
