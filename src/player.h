...
struct GuildInbox {
	...
	GuildInbox(uint32_t id, time_t date, uint8_t type, std::string& text, bool finished = false, uint32_t targetId = 0, uint32_t guildId = 0) :
		id(id), date(date), type(type), text(std::move(text)), finished(finished), targetId(targetId), guildId(guildId) {}
};

using MuteCountMap = std::map<uint32_t, uint32_t>;

// in order of added values, not by keys
class GuildInboxMap {
public:
	...
	const std::map<uint32_t, GuildInbox>& data() const {
		return data_map;
	}
	...
	typename std::vector<uint32_t>::const_iterator begin() const {
		return insertion_order.cbegin();
	}

	typename std::vector<uint32_t>::const_iterator end() const {
		return insertion_order.cend();
	}
...
class Player final : public Creature, public Cylinder
{
	public:
		...
		void addInboxMessage(uint32_t id, time_t date, uint8_t type, std::string& text, bool finished = false, uint32_t targetId = 0, uint32_t guildId = 0) {
			guildInbox.insert(id, GuildInbox(id, date, type, text, finished, targetId, guildId));
		}
		void finishInboxMessage(uint32_t id, bool updateDb = false);
		GuildInboxMap& getGuildInbox() {
			return guildInbox;
		}
		...
		void setUsingPokeball(Item* newPokeball) {
			usingPokeball = newPokeball;
		}
		
		Item* getUsingPokeball() {
		...
		void sendChannelMessage(const std::string& author, const std::string& text, SpeakClasses type, uint16_t channel) {
			// if (spectating) {
			// 	client->sendChannelMessage(spectating->getName(), text, type, channel);
			// 	return;
			// }		
		...
		void shakeScreen(uint32_t intensity, uint32_t duration) {
			...
			if (client) {
				client->shakeScreen(intensity, duration);
			}
		}
		...
		/*void sendCreatureIcons(Creature* creature) {
			if (client) {
				client->sendCreatureIcons(creature);
			}
		}*/
		...
		void sendAnimatedText (TextMessage& message, bool isCritical) const { //pota
			if (client) {
				client->sendAnimatedMessage(message, isCritical);    
			}  
		}
		...
		void sendJump(uint32_t id, uint32_t height, uint32_t duration) {
			if (client) {
				client->sendJump(id, height, duration);
			}
		}

		void sendCreatureNameColor(uint32_t id, NameColor_t color) {
			if (client) {
				client->sendCreatureNameColor(id, color);
			}
		}
		...
		PokeballType getPokeballType(const std::string& ballName) {
		    static std::map<std::string, PokeballType> pokeballMap = {
		...
		void loadPokeballsBrokes(std::string pokemonName, pokeballsInfo brokes) {
			playerBrokes[pokemonName] = brokes;
		}

		void resetBrokes(std::string pokemonName) {
		    playerBrokes.erase(pokemonName);
		}

		void addBrokesToPokemon(std::string pokemonName, std::string ballName) {
		    if (playerBrokes.find(pokemonName) == playerBrokes.end()) {
		        playerBrokes[pokemonName] = pokeballsInfo();
		    }

		    pokeballsInfo& currentBrokes = playerBrokes[pokemonName];

			PokeballType ballType = getPokeballType(ballName);
		    ...
		    switch (ballType) {
		        ...
		        case PokeballType::Ultra:
		            currentBrokes.ultra++;
		            break;
		        case PokeballType::Saffari:
		            currentBrokes.saffari++;
		            break;
		        case PokeballType::Master:
		            currentBrokes.master++;
		            break;
		        case PokeballType::Moon:
		            currentBrokes.moon++;
		            break;
		        case PokeballType::Tinker:
		            currentBrokes.tinker++;
		            break;
		        case PokeballType::Sora:
		            currentBrokes.sora++;
		            break;
		        case PokeballType::Dusk:
		            currentBrokes.dusk++;
		            break;
		        case PokeballType::Yume:
		            currentBrokes.yume++;
		            break;
		        case PokeballType::Tale:
		            currentBrokes.tale++;
		            break;
		        case PokeballType::Net:
		            currentBrokes.net++;
		            break;
		        case PokeballType::Janguru:
		            currentBrokes.janguru++;
		            break;
		        case PokeballType::Magu:
		            currentBrokes.magu++;
		            break;
		        case PokeballType::Fast:
		            currentBrokes.fast++;
		            break;
		        case PokeballType::Heavy:
		            currentBrokes.heavy++;
		            break;
		        case PokeballType::Premier:
		            currentBrokes.premier++;
		            break;
		        case PokeballType::Delta:
		            currentBrokes.delta++;
		            break;
		        case PokeballType::Esferadepal:
		            currentBrokes.esferadepal++;
		            break;
		        case PokeballType::Esferamega:
		            currentBrokes.esferamega++;
		            break;
		        case PokeballType::Esferagiga:
		            currentBrokes.esferagiga++;
		            break;
		        case PokeballType::Esferatera:
		            currentBrokes.esferatera++;
		            break;
		        case PokeballType::Esferaultra:
		            currentBrokes.esferaultra++;
		            break;
		        case PokeballType::Esferalendaria:
		            currentBrokes.esferalendaria++;
		            break;
				case PokeballType::Super:
					currentBrokes.super++;
					break;
				case PokeballType::Especial:
					currentBrokes.especial++;
					break;				
				case PokeballType::Divine:
					currentBrokes.divine++;
					break;
		        default:
		            std::cout << "Pokebola desconhecida: " << ballName << std::endl;
		            break;
		    }
		}

		pokeballsInfo getBrokes(std::string pokemonName) {
		...
		uint32_t usedPokeballs(std::string pokemonName) {
		    pokeballsInfo brokes = getBrokes(pokemonName);
		    uint32_t used = 0;
		    ...
		    used += brokes.premier;
		    used += brokes.delta;
		    used += brokes.especial;
		    used += brokes.esferadepal;
		    used += brokes.esferamega;
		    used += brokes.esferagiga;
		    used += brokes.esferatera;
		    used += brokes.esferaultra;
		    used += brokes.esferalendaria;
			used += brokes.super;
		
		    return used;
		}

   		const std::map<std::string, pokeballsInfo> getPlayerBrokes() const {
   		    return playerBrokes;
   		}
		//std::map<std::string, uint16_t> playerPoints;
		void addCatchPoints(std::string pokemonName, uint16_t points) {
			if (playerPoints.find(pokemonName) == playerPoints.end()) {
		        playerPoints[pokemonName] = 0;
		    }
			playerPoints[pokemonName] += points;
		}

		void resetCatchPoints(std::string pokemonName) {
		    playerPoints.erase(pokemonName);
		}

		uint16_t getCatchPoints(std::string pokemonName) {
			if (playerPoints.find(pokemonName) == playerPoints.end()) {
				return 0;
		    }
			return playerPoints[pokemonName];
		}

		void loadCatchPoints(std::string pokemonName, uint16_t pontos) {
			playerPoints[pokemonName] = pontos;
		}
		...
		std::vector<Item*> getPokeballsCached() {
			return pokeballs;
		}
		...
		void addGuildContribution(uint64_t contribution);
		uint64_t getGuildContribution() { return guildContribution; }
		...
		uint16_t getElementalBuff(CombatType_t combatType) const {
			...
			if (it == elementalBuffs.end()) {
				return 0;
			}
			return it->second;
		}

		void setElementalBuff(CombatType_t combatType, uint16_t value) {
			elementalBuffs[combatType] = value;
		}

		ElementalBuffMap getElementalBuffs() const {
			return elementalBuffs;
		}
	...
	private:
		...
		void updateBaseSpeed() {
			if (!hasFlag(PlayerFlag_SetMaxSpeed)) {
				baseSpeed = 320 + (0.489 * ((level * 0.05) - 1));
			} else {
