...
void Weapon::internalUseWeapon(Player* player, Item* item, Creature* target, [[maybe_unused]]  int32_t damageModifier) const
{
	if (scripted) {
		LuaVariant var;
		...
		damage.primary.type = getElementType();
		damage.primary.value = getElementDamage(player, target, item);
