...
class Condition
{
	public:
		Condition() = default;
		Condition(ConditionId_t id, ConditionType_t type, int32_t ticks, int64_t lastEffTicks, bool buff = false, uint32_t subId = 0, bool aggressive = false, bool saveCondition = false) :
			endTime(ticks == -1 ? std::numeric_limits<int64_t>::max() : 0),
			lastEffTicks(lastEffTicks),  // Usa o parâmetro corretamente
			subId(subId), ticks(ticks), conditionType(type), isBuff(buff), aggressive(aggressive), saveCondition(saveCondition), id(id) {}
		...
		virtual bool sendEff(Creature* creature, uint16_t effectId);
		...
		bool needSaveCondition() const {
			return saveCondition;
		}

		void setSaveCondition(bool save) {
			saveCondition = save;
		}
...
class ConditionBonus : public Condition 
{
	public:
		ConditionBonus(ConditionId_t id, ConditionType_t type, int32_t ticks, [[maybe_unused]] uint16_t bonus = 0, bool buff = false, uint32_t subId = 0, bool aggressive = false) :
			Condition(id, type, ticks, 0, buff, subId, aggressive) {}
		...
		ConditionBonus* clone() const override {
		...
		void setBonus(uint16_t newBonus) {
			bonus = newBonus;
		}
...
class ConditionDamage final : public Condition
{
	public:
		...
		bool addDamage(int64_t rounds, int64_t time, int64_t value);
...
{
	public:
		ConditionStatus(ConditionId_t id, ConditionType_t type, int32_t ticks, bool buff = false, uint32_t subId = 0, bool aggressive = false) :
			ConditionGeneric(id, type, ticks, buff, subId, aggressive) {}

		bool startCondition(Creature* creature) override;
		void addCondition(Creature* creature, const Condition* condition) override;

		ConditionStatus* clone() const override {
