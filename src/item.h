...
enum AttrTypes_t {
	...
	ATTR_COUNT = 36,
	ATTR_POKEBALL = 37,
	ATTR_POKEMONHOLDER = 38,
	ATTR_POKEMONHOLDERINFO = 39,
	ATTR_POKEMONHOLDERCREATUREID = 40,
	ATTR_UNIQUE_ITEM = 41,
	ATTR_ATTRIBUTE_HELDX = 42,
	ATTR_ATTRIBUTE_HELDY = 43,
	ATTR_ATTRIBUTE_HELDU = 44,
	ATTR_ATTRIBUTE_BLINK = 45,
...
class ItemAttributes
{
	...
	private:
		...
		const static uint64_t intAttributeTypes = ITEM_ATTRIBUTE_ACTIONID | ITEM_ATTRIBUTE_UNIQUEID | ITEM_ATTRIBUTE_DATE
			...
			| ITEM_ATTRIBUTE_FLUIDTYPE | ITEM_ATTRIBUTE_DOORID | ITEM_ATTRIBUTE_POKEMONHOLDERCREATUREID | ITEM_ATTRIBUTE_UNIQUE_ITEM 
			| ITEM_ATTRIBUTE_AUTOOPEN | ITEM_ATTRIBUTE_HELDX | ITEM_ATTRIBUTE_HELDY | ITEM_ATTRIBUTE_HELDU
			| ITEM_ATTRIBUTE_BLINK | ITEM_ATTRIBUTE_DECAYTO | ITEM_ATTRIBUTE_WRAPID;
		// ATRIBUTOS STRING
		const static uint32_t stringAttributeTypes = ITEM_ATTRIBUTE_DESCRIPTION | ITEM_ATTRIBUTE_TEXT | ITEM_ATTRIBUTE_WRITER
			| ITEM_ATTRIBUTE_NAME | ITEM_ATTRIBUTE_ARTICLE | ITEM_ATTRIBUTE_PLURALNAME | ITEM_ATTRIBUTE_SPECIAL | ITEM_ATTRIBUTE_POKEMONHOLDER
			| ITEM_ATTRIBUTE_POKEMONHOLDERINFO ;

	public:
		static bool isIntAttrType(itemAttrTypes type) {
			return (type & intAttributeTypes) != 0;
		}
		static bool isStrAttrType(itemAttrTypes type) {
			return (type & stringAttributeTypes) != 0;
		}
...
class Item : virtual public Thing
{
	public:
		...
		const int8_t getPokemonBoost() {
			const ItemAttributes::CustomAttribute* pokeballBoost = getCustomAttribute("pokeBoost");
			if (!pokeballBoost) {
				return 0;
			}
			if (const int64_t* boostPtr = boost::get<int64_t>(&pokeballBoost->value)) {
				return static_cast<int8_t>(*boostPtr);
			}
			return 0;
		}

		const std::string getPokemonName() {
			const ItemAttributes::CustomAttribute* pokeballName = getCustomAttribute("pokeName");
			if (!pokeballName) {
				return "";
			}
			if (const std::string* namePtr = boost::get<std::string>(&pokeballName->value)) {
				return *namePtr;
			}
			return "";
		}
		...
		std::string getSpecialAttribute(const std::string& key);
    	void setSpecialAttribute(const std::string& key, const std::string& value);
		...
		bool isPokeballItem() const {
			return items[id].isPokeball;
		}
