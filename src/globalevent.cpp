...
void GlobalEvents::createHolders(Item* pokeball) const
{
	if (pokeball) {
		createHolders(pokeball);
	}
}
...
bool GlobalEvent::configureEvent(const pugi::xml_node& node)
{
	...
	if ((attr = node.attribute("time"))) {
		...
		} else if (strcasecmp(value, "holders") == 0) {
			eventType = GLOBALEVENT_HOLDERS;
...
{
	switch (eventType) {
		case GLOBALEVENT_HOLDERS: return "onHolders"; 
...
void GlobalEvent::createHolder(Item* pokeball) const
{
	if (!scriptInterface->reserveScriptEnv()) {
		std::cout << "[Error - GlobalEvent::executeEvent] Call stack overflow" << std::endl;
		return;
