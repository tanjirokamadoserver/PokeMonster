...
bool Map::loadMap(const std::string& identifier, bool loadHouses)
{
	...
	if (loadHouses) {
		...
		IOMapSerialize::loadHouseHolders();
	}
	return true;
}
...
bool Map::placeCreature(const Position& centerPos, Creature* creature, bool extendedPos/* = false*/, bool forceLogin/* = false*/, bool isLogin/* = false*/)
{
	bool foundTile;
	bool placeInPZ;
	...
	if (isLogin && creature->getPlayer()->isFlying() && !foundTile) {
		if (tile) {
			foundTile = true;
		} else {
			tile = new StaticTile(centerPos.x, centerPos.y, centerPos.z);
			g_game.map.setTile(centerPos, tile);
			foundTile = true;
		}
	}
...
uint32_t Map::cleanZone(Zone* zoneToClean) const
{
    uint64_t start = OTSYS_TIME();
    size_t tiles = 0;

    std::vector<Item*> toRemove;
    for (auto tileList : zoneToClean->getTiles()) {
        if (!tileList) {
            continue;
        }
        ++tiles;
    ...
    size_t count = toRemove.size();
    std::cout << ">> ZONE " << zoneToClean->getId() << " CLEANED: Removed " << count << " item" << (count != 1 ? "s" : "")
        << " from " << tiles << " tile" << (tiles != 1 ? "s" : "") << " in "
        << (OTSYS_TIME() - start) / (1000.) << " seconds." << std::endl;
    return count;
}