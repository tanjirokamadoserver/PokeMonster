...
class Game
{
	public:
		...
		/**
		  * Returns a duelist based on the unique creature identifier
		  * \param id is the unique npc id to get a npc pointer to
		  * \returns A NPC pointer to the npc
		  */
		Duelist* getDuelistByID(uint32_t id);
		...
		/**
		  ...
		  * \param isLogin If true, the creature is being placed on the map because of a login
		  */
		bool internalPlaceCreature(Creature* creature, const Position& pos, bool extendedPos = false, bool forced = false, bool isLogin = false);
		...
		/**
		  ...
		  * \param isLogin If true, the creature is being placed on the map because of a login
		  */
		bool placeCreature(Creature* creature, const Position& pos, bool extendedPos = false, bool forced = false, bool isLogin = false);
		...
		void startJump(Creature* creature, uint32_t height, uint32_t duration);

		void sendCreatureNickname(Creature* creature, const std::string& nickname);
		void sendFirstTitle(Creature* creature, const std::string& title, const std::string& font, const std::string& color);
		...
		void updateCreatureNameColor(const Creature* player, NameColor_t color);
	...
	//	void sendCreatureIcons(Creature* creature);
		...
		const std::map<uint32_t, Duelist*>& getDuelists() const { return duelists; }
		...
		void loadGuilds();
		const std::unordered_map<uint32_t, Guild*>& getGuilds() const { return guilds; }

		void addDuelist(Duelist* duelist);
		void removeDuelist(Duelist* duelist);
		...
		std::string humanCase(const std::string& input);
		...
		void addZone(uint16_t zoneId, const std::vector<zoneInfos>& positions, const std::string& zoneName) {
		    Zone* newZone = new Zone(zoneId);
		    newZone->setName(zoneName);

		    for (const auto& position : positions) {
		        newZone->addPositions(position);
		    }

		    gameZones[zoneId] = newZone;
		}

		Zone* getZoneById(uint16_t id) {
		    auto it = gameZones.find(id);
		    if (it != gameZones.end()) {
		        return it->second;
		    }
		    return nullptr;
		...
		uint16_t getZoneIdByPosition(const Position& playerPos) {
		    for (const auto& item : gameZones) {
		        Zone* zone = item.second;
		        if (zone->isPositionInsideZone(playerPos)) {
		            return item.first;
		        }
		    }
		    return 0;
    	...
    	std::size_t getZonesNumber() const {
		...
		void addDuelistType(const std::string& name, const DuelistInfo& info) {
		    duelistsType[name] = info;
		}

		const DuelistInfo& getDuelistType(const std::string& name) {
		    auto it = duelistsType.find(name);
		    if (it != duelistsType.end()) {
		        return it->second;
		    }
		    return duelistsType["Sombrio"];
		}
	...
	private:
		...
		std::map<std::string, DuelistInfo> duelistsType;
		...
		std::unordered_map<uint16_t, Zone*> gameZones;
		...
		std::map<uint32_t, Duelist*> duelists;
